---
title: "Account Search API (Beta)"
api: "GET https://api-c.walletlabels.xyz/solana/search"
description: "Retrieve user-friendly labels for Solana EOAs (Externally Owned Accounts) and Smart Contracts. Labels can be incredibly useful for categorizing your Solana interactions."
---

### Authorization

<ResponseField
  name="x-api-key"
  type="string"
  default="demo-api-key-x-00000"
  initialValue="demo-api-key-x-00000"
  required
>
  Your API key for authentication.
</ResponseField>

### Body

<ParamField query="searchtext" type="string" required>
  Specify the Solana attribute (label, Sub_label or address_name) for which you
  want to retrieve labels.
</ParamField>
<ParamField query="limit" type="string">
  Specify number of shown results.
</ParamField>
<ParamField query="offset" type="string">
  Specify the starting point in the list of results for pagination. 
  e.g. `offset=0`, `offset=1`, etc..
</ParamField>

### Response

<ResponseField name="data" type="array">
  An array that encapsulates the labels corresponding to the Solana address provided.

<Expandable title="Label Object">

<ResponseField name="address_name" type="string">
  The designated name associated with the specific Solana address.
</ResponseField>

{" "}

<ResponseField name="address" type="string">
  The Solana address for the label.
</ResponseField>

{" "}

<ResponseField name="label_type" type="string">
  Categories like 'nft', 'gaming', etc., to define the nature of the label.
</ResponseField>

{" "}

<ResponseField name="label_subtype" type="string">
  Additional attributes, like nf_token_contract or aggregator_contract, for a
  more granular understanding of the label.
</ResponseField>

{" "}

<ResponseField name="label" type="string">
  The general name or title of the label.
</ResponseField>

  </Expandable>
</ResponseField>

<RequestExample>

```bash Example Request
curl --location --request GET 'https://api-c.walletlabels.xyz/solana/search?searchtext=nft&limit=1' \
--header 'Content-Type: application/json' \
--header 'x-api-key: demo-api-key-x-00000'
```

</RequestExample>

<ResponseExample>

```json Response
{
  "data": [
    {
      "address": "8jJo9ZFB9QiDA2UP7TAK8Vr4JxUVTiCLoVZHjauK8g2c",
      "address_name": "nft nft nft",
      "label_type": "nft",
      "label_subtype": "nft",
      "label": "nft nft nft"
    }
  ]
}
```

</ResponseExample>
